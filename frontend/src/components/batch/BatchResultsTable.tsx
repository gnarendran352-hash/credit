...
import { ChevronUp, ChevronDown, Search, AlertTriangle, CheckCircle, Eye } from 'lucide-react';
...
const BatchResultsTable: React.FC<BatchResultsTableProps> = ({ predictions }) => {
  const [sortKey, setSortKey] = useState<SortKey>('risk_score');
  const [sortOrder, setSortOrder] = useState<SortOrder>('desc');
  const [filter, setFilter] = useState<FilterKey>('all');
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [selectedTransaction, setSelectedTransaction] = useState<BatchResultWithClass | null>(null);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                ...
                <th className="text-left py-2.5 px-3 text-xs text-white/30 font-medium">Prediction</th>
                <th className="text-left py-2.5 px-3 text-xs text-white/30 font-medium">
                  <button
                ...
                <th className="text-left py-2.5 px-3 text-xs text-white/30 font-medium">Actions</th>
              </tr>
            </thead>
            <tbody>
              {paginated.map((p, i) => (
                ...
                >
                  ...
                  <td className="py-2.5 px-3">
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all text-xs"
                    >
