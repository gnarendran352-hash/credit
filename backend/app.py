...
async def get_model_metrics():
    return {
        ...
        "model_type": type(model).__name__ if model is not None else "RandomForestClassifier",
        "training_samples": 227845,
        "feature_count": model.n_features_in_ if model is not None and hasattr(model, 'n_features_in_') else 30,
        "timestamp": datetime.now().isoformat()
    }
